...
typedef struct
{					// estrutura que vai ter o numero primo e o pid
...
{															 // pipe, numero de primos a calcular, nome do ficheiro, ponteiro para o ficheiro,
	...
	while (g != m)
	{
		...
		if (buf.numero == -1) // quando o número é -1, significa que o processo filho terminou de encher o pipe
			...
			l++;
		if (buf.numero != -1 && p1 >= l)
		{ // se n estivermos no fim do pipe e o número total de primos ainda n tiver sido alcançado
			if (lvalor == NULL)
			{ // se o ficheiro n for definido na linha de comandos, imprimir no ecrã
				printf("[PRIMO: %d]\t [PROCESSO: %d]\n",
		...
		{ // se já tivermos atingido o número de primos desejados
			if (buf.numero == -1 && p1 <= l)
			{ // se estivermos no fim de um processo, e se já tivermos os primos
				if (lvalor == NULL)
...
int main(int argc, char **argv)
{
	...
	if (pipe(pdes) == -1)
	{ // inicialização do pipe, se n for possível, dar erro
		perror("Error");
	}
	...
	*/
	...
	while ((o = getopt(argc, argv, "l:g:t:n:p:h")) != -1)
		...
		case 'h': // todos os outros case têm breaks para que quando se ponha a flag -h nada mais aconteça
			...
			printf("-n\tNúmero de processos a criar pelo processo detective\n");
			printf("-t\tComprimento dos sub-intervalos para atribuir a cada processo-filho\n");
			printf("-l\tFicheiro que irá conter os vários números primos\n");
			printf("-g\tGama limiteInferior:limiteSuperior\n");
			exit(0);
		}

	if (lvalor != NULL)
	{							 // se o ficheiro for especificado, então abre-se de imediato
		fp = fopen(lvalor, "w"); // parte-se do principio que o utilizador não quer binário, e que não quer acomular resultados (w)
		if (fp == NULL)
		{ // informa o utilizador se n for possível abrir o ficheiro
			printf("Houve um erro ao abrir o ficheiro para escrita\n");
			exit(6);
		}
	}
	...
	{
		printf("Deve especificar um dos valores, ou o do número de processos, ou o tamanho do sub-intervalo\n");
		exit(2);
	}

	if (nvalor != NULL)
	{ // tratamento para quando n são dados valores
		n1 = atoi(nvalor);
	}
	else
	{
	...
	if (gvalor != NULL)
	{
		...
		for (i = 0; gvalor[i] != ':'; i++)		 // tirar o valor inferior até aos :
			num1[i] = gvalor[i];
		i++;
		for (j = 0; gvalor[i] != '\0'; i++, j++)
		{ // tirar o valor superior até ao fim
			num2[j] = gvalor[i];
		}
	...
	{ // se o valor da gama n�o for especificado, usar os valores que estão em getopt.h
		nb = VALORB;
		na = VALORA;
	}
	...
	{
		printf("Especifique apenas ou o número de processos, ou o tamanho do subintervalo\n");
		exit(2);
	}

	if ((nb) > (na))
	{													 // até se podia por para trocar
	...
	if (n1 < 1)
	{
		printf("O námero de processos deve ser sempre maior que 0\n");
		exit(2);
	}

	if (n1 > ((nb + na) / 2))
	{ // não consegui gerar um melhor algoritmo para dividir o tamanho da gama pelos processos
		printf("O número de processos deve ser igual ou menor que o tamanho da gama\n");
		exit(2);
	}

	if (p1 < 1)
	{
		printf("O número de primos a calcular deve ser sempre maior que 0\n");
		exit(2);
	}

	if (t1 < 1)
	{
		printf("O comprimento do subintervalo deve ser sempre maior que 0\n");
		exit(2);
	}

	if (t1 > (na - nb))
	{
		printf("O tamanho do subintervalo deve ser inferior à gama de valores\n");
		exit(2);
	}
	...
	while (n1 > 0)
	{
		...
		if (pid == 0)
		{
			// Each child process now uses its own local copy of nb and na
			printf("\nFrom %d to %d\n", nb, na);
			calcprimos(nb, na);
			exit(0); // Important: child processes should exit after their work
		}
		else if (pid > 0)
		{
			m++; // Increment for pipe reading
